...
def run(args):
    ...
    with tf.variable_scope('model_generator'):
        gen_output = _wrap_update_ops(args.generator.generator, gen_input_seed, is_training, gen_input_class, args.hyperparam.IMAGE_DIM)
    ...
    with tf.name_scope('train_ops'):
        ...
        train_dis = otw.adam_updates(discriminator_variables, dis_loss_regularized, lr=args.hyperparam.LR_DIS)
        train_cls = otw.adam_updates(discriminator_variables, cls_loss_regularized, lr=args.hyperparam.LR_CLS)
        train_gen = otw.adam_updates(generator_variables, gen_loss_regularized, lr=args.hyperparam.LR_GEN)
        
        if args.hyperparam.WGAN_ENABLE:
            logger.info("Building WGAN optimizer.")
            with tf.name_scope('wgan'):
                train_dis = otw.adam_updates(discriminator_variables, dis_loss_wgan, lr=args.hyperparam.LR_DIS)
                train_gen = otw.adam_updates(generator_variables, gen_loss_wgan, lr=args.hyperparam.LR_GEN)
